  ...
  class PostErasureTransformer(unit: CompilationUnit) extends TypingTransformer(unit) {
    override def transform(tree: Tree) = {
      ...
      def binop(lhs: Tree, op: Symbol, rhs: Tree) =
        localTyper typed (Apply(Select(lhs, op.name) setPos tree.pos, rhs :: Nil) setPos tree.pos)

      super.transform(tree) setType elimErasedValueType(tree.tpe) match {
        case AsInstanceOf(v, tpe) if v.tpe <:< tpe => v                  // x.asInstanceOf[X]       ==> x
        case ValueClass.BoxAndUnbox(v)             => v                  // (new B(v)).unbox        ==> v
