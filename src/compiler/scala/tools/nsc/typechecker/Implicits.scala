            ...
            ranked.foldLeft((nonInductive: SearchResult, None: Option[ImplicitInfo])) {
              ...
              case (acc@(prevResult, Some(prevInfo)), info) if prevResult.isSuccess && prevInfo != info && !improves(prevInfo, info) =>
                ...
                if(ambigResult.isFailure)
                  ...
                  if(reportAmbiguous)
                    AmbiguousImplicitError(prevInfo, info, "both", "and", "")(false, pt, tree)(context)
            ...
            case MethodType(params, mt) =>
              ...
              for(param <- params) {
                ...
                val res =
                  ...
                  else inferImplicit(fun, paramTp, reportAmbiguous = false, isView = false, context = context, saveAmbiguousDivergent = true, pos = tree.pos)
